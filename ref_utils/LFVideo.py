...
class LFVideo(Dataset):
    ''' LFVideo Dataset by Yaping'''

    def __init__(self, data_path_corrupted, data_path_clean, data_path_MDSR, data_list_file, frame_num=9, angular_num=8,
        ...
        self.frame_list = ['%02d/' % i for i in range(1, frame_num+1)]
        self.angular_list = [str(i) + '_' + str(i) + '.png' for i in range(angular_num)]
        # transform
        self.transform = transform
        self.crop = None
        self.is_train = is_train
        self.N = len(self.folder_list_clean)  # sequences number
        self.frame_num = frame_num  # frame number
        self.angular_num = angular_num  # frame number
        # self.N = self.M * self.K    # total number
        # get shape
        self.require_seqid = require_seqid
        if self.require_seqid:
            buff, seqid = self.__getitem__(0)
            _, self.H, self.W = buff['input_img1_SR'].size()
        else:
            _, self.H, self.W = self.__getitem__(0)['input_img1_SR'].size()
    ...
    def get_frame(self, m, f, a, mode='LR'):
        ''' return a 3-D [3,H,W] float32 [0.0-1.0] tensor '''
        # print (len(self.file_list),f)
        if mode == 'HR':
            filename = self.folder_list_clean[m] + self.frame_list[f] + self.angular_list[a]
            image = Image.open(filename)
        elif mode == 'LR':
            filename = self.folder_list_corrupted[m] + self.frame_list[f] + self.angular_list[a]
            image = Image.open(filename)
        elif mode == 'SR':
            filename = self.folder_list_MDSR[m] + self.frame_list[f] + self.angular_list[a]
            image = Image.open(filename)
        elif mode == 'Gray':
            filename = self.folder_list_clean[m] + self.frame_list[f] + self.angular_list[a]
            image = Image.open(filename)
    ...
    def __getitem__(self, idx):
        if self.is_train:
            m = idx
            frame_id = random.randint(1, self.frame_num-1)
            angular_id = random.randint(1, self.angular_num-1)
        else:
            m = idx
            frame_id = self.frame_num-1
            angular_id = self.angular_num-1
        # self.window_size = self.frame_num_list[idx]

        sample = dict()

        sample['input_img1_LR'] = self.get_frame(m, 0, 0, mode='LR')
        if self.transform:
            sample['input_img1_LR'] = self.transform(sample['input_img1_LR'])

        sample['input_img1_HR'] = self.get_frame(m, 0, 0, mode='HR')
        if self.transform:
            sample['input_img1_HR'] = self.transform(sample['input_img1_HR'])

        sample['input_img1_SR'] = self.get_frame(m, 0, 0, mode='SR')
        if self.transform:
            sample['input_img1_SR'] = self.transform(sample['input_img1_SR'])

        sample['input_img2_HR'] = self.get_frame(m, frame_id, angular_id, mode='HR')
        if self.transform:
            sample['input_img2_HR'] = self.transform(sample['input_img2_HR'])

        sample['input_img2_Gray'] = self.get_frame(m, frame_id, angular_id, mode='Gray')
        if self.transform:
            sample['input_img2_Gray'] = self.transform(sample['input_img2_Gray'])

        sample['input_img1_Gray'] = self.get_frame(m, 0, 0, mode='Gray')
        if self.transform:
...
if __name__ == "__main__":
    ...
    dataset = LFVideo(data_path_corrupted, data_path_clean, data_path_MDSR, data_list_file, frame_window_size=2,
                           transform=composed)
