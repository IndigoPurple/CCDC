...
class Flower(Dataset):
    ''' Flower Dataset by Yaping'''

    def __init__(self, data_path_corrupted, data_path_clean, data_path_MDSR, data_list_file, angular_num=8,
        ...
        self.file_list = [ str(i) + '_' + str(i) + '.png' for i in range(angular_num)]
        # transform
        ...
        if is_train:
            self.N = self.M * self.angular_num  # total number
        else:
            self.N = self.M
        # get shape
    ...
    def __getitem__(self, idx):
        if self.is_train:
            m = idx // self.angular_num
            img2_id = idx % (self.angular_num-1)
        else:
            m = idx
            img2_id = self.angular_num-2
        # self.window_size = self.frame_num_list[idx]

        sample = dict()

        sample['input_img1_LR'] = self.get_frame(m, 0, mode='LR')
        if self.transform:
            sample['input_img1_LR'] = self.transform(sample['input_img1_LR'])

        sample['input_img1_HR'] = self.get_frame(m, 0, mode='HR')
        if self.transform:
            sample['input_img1_HR'] = self.transform(sample['input_img1_HR'])

        sample['input_img1_SR'] = self.get_frame(m, 0, mode='SR')
        if self.transform:
            sample['input_img1_SR'] = self.transform(sample['input_img1_SR'])

        sample['input_img2_HR'] = self.get_frame(m, img2_id+1, mode='HR')
        if self.transform:
            sample['input_img2_HR'] = self.transform(sample['input_img2_HR'])

        sample['input_img2_Gray'] = self.get_frame(m, img2_id+1, mode='Gray')
        if self.transform:
            sample['input_img2_Gray'] = self.transform(sample['input_img2_Gray'])

        sample['input_img1_Gray'] = self.get_frame(m, 0, mode='Gray')
        if self.transform:
...
if __name__ == "__main__":
    ...
    dataset = Flower(data_path_corrupted, data_path_clean, data_path_MDSR, data_list_file, frame_window_size=2,
                           transform=composed)
