...
class UNet_decoder_res(nn.Module):

    def __init__(self, activation = 'selu' , init_type = 'w_init', out_channel=3):
        super(UNet_decoder_res, self).__init__()
    ...
    def forward(self,res_conv1, res_conv2, res_conv3, res_conv4):

        warp_deconv4 = self.warp_deconv4(res_conv4)

        concat1 = torch.cat((warp_deconv4,res_conv3),1)
        warp_deconv3 = self.warp_deconv3(concat1)

        concat2 = torch.cat((warp_deconv3,res_conv2),1)
        warp_deconv2 = self.warp_deconv2(concat2)

        concat3 = torch.cat((warp_deconv2,res_conv1),1)
        post_fusion1 = self.post_fusion1(concat3)
...
class UNet_decoder_gray(nn.Module):

    def __init__(self, activation = 'selu' , init_type = 'w_init', out_channel=3):
        super(UNet_decoder_gray, self).__init__()
        ...
        self.post_fusion2 = conv_activation(67, 64, kernel_size = 5, stride = 1, padding = 2, activation = activation,init_type = init_type)
    ...
    def forward(self,LR_conv1, LR_conv2, LR_conv3, LR_conv4, warp_conv1, warp_conv2, warp_conv3, warp_conv4,
                diff_conv1, diff_conv2, diff_conv3, diff_conv4, diff):
        concat0 = torch.cat((LR_conv4, warp_conv4, diff_conv4), 1)
        warp_deconv4 = self.warp_deconv4(concat0)

        concat1 = torch.cat((warp_deconv4, LR_conv3, warp_conv3, diff_conv3), 1)
        warp_deconv3 = self.warp_deconv3(concat1)

        concat2 = torch.cat((warp_deconv3, LR_conv2, warp_conv2, diff_conv2), 1)
        warp_deconv2 = self.warp_deconv2(concat2)

        concat3 = torch.cat((warp_deconv2, LR_conv1, warp_conv1, diff_conv1), 1)
        post_fusion1 = self.post_fusion1(concat3)

        concat4 = torch.cat((post_fusion1, diff), 1)
        post_fusion2 = self.post_fusion2(concat4)
